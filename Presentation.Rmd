...
output: 
  ioslides_presentation:
    logo: "your_university_logo.png" 
    widescreen: true
    smaller: false # Allow for more text, adjust if too cramped
    transition: default # Slower for readability
    css: styles.css 
...
```{r interactive_density_plot, fig.width=9, fig.height=6}
# Use ggplot2's built-in diamonds dataset
diamonds_subset <- ggplot2::diamonds %>%
  filter(cut %in% c("Fair", "Ideal", "Premium")) %>%
  select(price, cut, carat)

# Make 'cut' an ordered factor
diamonds_subset$cut <- factor(
  diamonds_subset$cut, 
  levels = c("Fair", "Premium", "Ideal")
)
...
p <- ggplot(diamonds_subset, aes(
  ...
  labs(
    title = "Interactive Price Distribution by Diamond Cut (Log Scale)",
    subtitle = "Density of diamond prices for selected cuts",
...
```{r db_sample_and_plot_density_interactive_wordy, fig.width=9, fig.height=6}
# Fetch data for selected cuts, focusing on price, cut, and carat.
# This data is collected into R for client-side plotting.
diamonds_subset_for_density <- diamonds_db_ref %>%
  filter(cut %in% c("Fair", "Ideal", "Premium")) %>% 
  select(price, cut, carat) %>%
  collect() 

# Ensure 'cut' is an ordered factor for consistent faceting and legend.
diamonds_subset_for_density$cut <- factor(
  diamonds_subset_for_density$cut, 
  levels = c("Fair", "Premium", "Ideal") 
)

# Create the density plot using ggplot2, preparing for plotly conversion.
density_plot_gg <- ggplot(diamonds_subset_for_density, 
                          aes(x = price, fill = cut,
                              text = paste("Cut:", cut, "<br>Price:", dollar(price)))) +
  geom_density(alpha = 0.65, adjust = 1.5) + # `adjust` controls smoothness
  scale_x_log10(
    labels = dollar_format(prefix="$", suffix="", big.mark=","), 
    breaks = c(500, 1000, 2000, 5000, 10000, 18000) # Custom breaks for log scale
  ) + 
  scale_fill_viridis_d(option = "magma") +
  labs(
    title = "Interactive Price Distribution by Diamond Cut (Log Scale)",
    subtitle = "Illustrating density of diamond prices for selected cuts",
    x = "Price (USD, Logarithmic Scale)",
    y = "Density",
    fill = "Cut Quality"
  ) +
  facet_wrap(~cut, ncol = 1, scales = "free_y") + # Separate plot for each cut, free y-axis for better comparison of shapes
  theme_light(base_size = 12) +
...
interactive_density_plot <- ggplotly(density_plot_gg, tooltip = c("x", "fill")) # Default tooltip for density can be tricky; text aesthetic can be used if specific point data is desired.

print(interactive_density_plot)
```
